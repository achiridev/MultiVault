...
`TenantService.create` (orquestador, no transaccional) coordina el aprovisionamiento de organización en tres etapas (ADR-0004): `TenantProvisioningService.initialize` (TX 1: `tenant` PENDING_PROVISIONING + `subscription` ACTIVE + `tenant_usage` + `tenant_member` + `identity_provider` obligatorio, ADR-0006), `TenantSchemaProvisioner.provision` (CREATE SCHEMA + Flyway por tenant, fuera de transacción) y `TenantProvisioningService.activate` (TX 2: tenant ACTIVE + `current_plan_id`, API key inicial del admin vía `ApiKeyService.createInitial`, auditoría `TENANT_CREATED`). `TenantService.updateIdentityProvider` actualiza la config OIDC vía `PUT /api/v1/tenants/identity-provider` (auditoría `TENANT_IDENTITY_PROVIDER_UPDATED`); el `tenantId` lo provee `CurrentTenant.serviceTenantId()` desde el principal SERVICE (ADR-0012). El mapeo DTO ↔ Entidad lo generan mappers MapStruct (`tenant/mapper/`, `subscription/mapper/`, `document/mapper/`) según ADR-0002. `ApiKeyService.createInitial` genera la key raw (`mv_live_` + 40 hex), la persiste como SERVICE con scope `*` (llave maestra, ADR-0012), guarda solo su hash SHA-256 y la devuelve una única vez.

`TenantLifecycleService` gestiona el ciclo de vida del tenant vía `PUT /api/v1/tenants/status` (ADR-0009; solo credencial SERVICE, ADR-0012): `updateStatus` valida la transición, aplica los efectos colaterales (cancelar/suscripción, revocar keys, desactivar miembros) y registra auditoría (`TENANT_CANCELLED`, `TENANT_SUSPENDED`, `TENANT_REINSTATED`). Cancelación es permanente; suspensión es reversible (reinstate solo desde `SUSPENDED`).
